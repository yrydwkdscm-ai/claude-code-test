...
// V3-V5 (see render-video.v3.mjs's header comment for why this avoids
// the white-flash bug). V6 builds on V5's Jagumi character + background
// "effects" system, adding: bigger/scene-tuned character sizing, a
// text-swap mechanism (80,000,000G -> 8000万G), countup milestone
// pulses, a reel-stop effect, a red-neon effect, and per-number decor
// (rising arrow, circular gauge, falling arrow).
//
// ---- Where the Jagumi character assets came from (unchanged since V5) ----
...
// with isnet-anime. So V6 (like V5) uses ONLY those seven crops as
// character assets -- no new character art was generated, and the busy
// full hero splash art was not used. Crop boxes (in the original
// 1230x1278 sheet) and resulting files:
...
import { timeline, CANVAS } from "../scenes/timeline.v6.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(__dirname, "..");
const TEMPLATE_PATH = path.join(ROOT, "template", "scene.v6.html");
const FONT_BOLD = path.join(ROOT, "assets", "fonts", "NotoSansJP-Bold.v6.subset.woff2");
const FONT_BLACK = path.join(ROOT, "assets", "fonts", "NotoSansJP-Black.v6.subset.woff2");
const CHAR_DIR = path.join(ROOT, "assets", "characters");
const RENDER_TMP_DIR = path.join(ROOT, "frames", "v6", "_html");
const VIDEO_DIR = path.join(ROOT, "frames", "v6", "_raw");
const FRAMES_DIR = path.join(ROOT, "frames", "v6");
const CHROMIUM_PATH =
...
function renderCountup(line, defaultDelay) {
  ...
  const milestones = (line.milestones || []).join(",");
  return `<span class="role-${line.displayRole} countup" data-count-to="${line.to}" data-count-suffix="${escapeHtml(
    line.suffix || ""
  )}" data-count-duration="${line.duration ?? 2}" data-count-delay="${delay}" data-milestones="${milestones}">0${escapeHtml(
    line.suffix || ""
  )}</span>`;
}

const ENTER_CLASSES = new Set(["fadeUp", "riseUp", "dropSlam"]);

function renderLine(line) {
  const delay = line.delay ?? 0;
  const enterClass = ENTER_CLASSES.has(line.enter) ? ` enter-${line.enter}` : "";
  ...
  const swapAttrs = line.swapTo
    ? ` data-swap-to="${escapeHtml(line.swapTo)}" data-swap-delay="${line.swapDelay ?? 0.5}"${
        line.swapVars ? ` data-swap-vars="${escapeHtml(varsToStyle(line.swapVars).slice(1))}"` : ""
      }`
    : "";

  const { burstHtml, spanHtml: spanHtmlRaw } = renderAccentSpan(
    {
      role: line.role,
      text: line.text,
      pop: line.pop,
  ...
  // Splice the swap data-attributes onto the rendered span (simplest way
  // to keep renderAccentSpan the single place that builds the tag).
  const spanHtml = swapAttrs ? spanHtmlRaw.replace(/^(<span\b)/, `$1${swapAttrs}`) : spanHtmlRaw;
...
function renderJagumi(cfg, charUrls) {
  ...
  const shakeAttr = cfg.idle === "shake" ? ` data-shake="${cfg.shakeDelay ?? 0}"` : "";
  const qmark = cfg.questionMark ? `<div class="jg-qmark">？</div>` : "";
  // "指差す" flourish: a small pulsing sparkle near her raised hand,
  // rather than a different pose asset (none of the 7 crops show a
  // clean pointing arm at this framing -- see the render script's
  // header comment on why the busy hero splash art wasn't used).
  const point = cfg.point ? `<div class="jg-point-spark"></div>` : "";
  return `<div class="jagumi-slot mode-${cfg.mode} side-${cfg.side}">
      <div class="jagumi-bob">
        <img class="jagumi-img enter-${enter}" src="${src}" style="--jg-size:${cfg.size}px"${shakeAttr} />
        ${qmark}
        ${point}
      </div>
    </div>`;
...
function renderDecor(kind, gaugeValue) {
  if (kind === "arrow-up" || kind === "arrow-down") {
    const flip = kind === "arrow-down";
    // simple chevron/arrow polygon pointing up; flipped via transform for down
    const path = "M170,640 L170,260 L60,260 L260,20 L460,260 L350,260 L350,640 Z";
    return `<div class="decor-layer"><svg class="decor-arrow ${kind}" viewBox="0 0 520 660" style="${
  ...
  if (kind === "gauge") {
    const r = 210;
    const c = 2 * Math.PI * r;
    const frac = Math.min(1, gaugeValue ?? 1);
    const offset = c * (1 - frac);
    return `<div class="decor-layer"><svg class="decor-gauge" width="480" height="480" viewBox="0 0 480 480">
      <circle class="track" cx="240" cy="240" r="${r}" />
      <circle class="fill" cx="240" cy="240" r="${r}" stroke-dasharray="${c.toFixed(1)}" stroke-dashoffset="${offset.toFixed(
      1
    )}" />
    </svg></div>`;
  }
...
// How much vertical room (px, from the bottom) to reserve for a
// hero-mode character so she never overlaps the text safe-area above
// her. Scales with her configured height plus a fixed buffer.
function heroSafeBottom(jagumi) {
  if (!jagumi || jagumi.mode !== "hero") return 300;
  return Math.min(1000, Math.round(jagumi.size * 1.1 + 260));
}
...
function renderScene(scene, charUrls) {
  ...
  const decorHtml = eff.decor ? renderDecor(eff.decor, eff.gaugeValue) : "";
  const dataAttrs = [
    eff.dim ? `data-effect-dim="1"` : "",
    eff.reelTurbo ? `data-effect-reel-turbo="1"` : "",
    eff.reelStop ? `data-effect-reel-stop="1"` : "",
    eff.graphFreeze ? `data-effect-graph-freeze="1"` : "",
    eff.neonRed ? `data-effect-neon-red="1"` : "",
    eff.digitsBurst ? `data-effect-digits-burst="1"` : "",
    ...
    .filter(Boolean)
    .join(" ");
  const safeAreaStyle = `--line-gap:${scene.gap}${
    scene.heroText ? `;--safe-bottom:${heroSafeBottom(scene.jagumi)}px` : ""
  }`;
...
async function main() {
  if (!existsSync(FONT_BOLD) || !existsSync(FONT_BLACK)) {
    throw new Error(
      "V6 font files missing under assets/fonts/. Expected NotoSansJP-Bold.v6.subset.woff2 and NotoSansJP-Black.v6.subset.woff2."
    );
  }
  ...
  if (process.argv.includes("--html-only")) {
    console.log(`[render-v6] --html-only: wrote ${htmlPath}, skipping recording.`);
    return;
  }
  ...
  try {
    ...
    console.warn("[render-v6] CDP setBackgroundColor unavailable, relying on CSS + leadIn trim:", err.message);
  }
  ...
  console.log(
    `[render-v6] done: ${finalWebm} (totalDuration ${totalDuration}s, leadIn ${leadIn.toFixed(3)}s)`
  );
}
