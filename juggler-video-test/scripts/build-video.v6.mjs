...
// file's header comment) applied to Version 6's recording.
...
const OUTPUT_NAME = "juggler-test-30s-v6.mp4";
...
async function main() {
  ...
  if (!Number.isFinite(sourceDuration) || sourceDuration < required) {
    throw new Error(
      `Raw recording is only ${sourceDuration}s but trimming needs ${required.toFixed(
        ...
        `Increase RECORD_PAD_SEC in render-video.v6.mjs and re-render.`
    );
  }
  ...
  console.log(
    `[build-v6] done: ${outputPath} (trimmed ${ss}s lead-in, output duration ${outDuration}s, expected ${totalDuration}s) ${
