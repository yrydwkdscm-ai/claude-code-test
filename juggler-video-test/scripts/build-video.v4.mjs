...
// file's header comment) applied to Version 4's recording.
...
const OUTPUT_NAME = "juggler-test-30s-v4.mp4";
...
async function main() {
  ...
  if (!Number.isFinite(sourceDuration) || sourceDuration < required) {
    throw new Error(
      `Raw recording is only ${sourceDuration}s but trimming needs ${required.toFixed(
        ...
        `Increase RECORD_PAD_SEC in render-video.v4.mjs and re-render.`
    );
  }
  ...
  console.log(
    `[build-v4] done: ${outputPath} (trimmed ${ss}s lead-in, output duration ${outDuration}s, expected ${totalDuration}s) ${
